# Ticket 203: Implement SHA256 Hashing
...
Implement SHA256 hashing in `esrp-canonical/src/hash.rs`. This provides the foundation for artifact verification, payload hashing, and content-addressed storage.

## Context

SHA256 hashing is used throughout ESRP for:
- Computing `payload_hash` for idempotency
- Artifact verification via `sha256` field
- Content-addressed storage
- Deduplication

The hash is always represented as a 64-character lowercase hex string.

## Tasks

1. Implement `hash_bytes()` function for raw byte hashing
2. Implement `hash_canonical()` for combined canonicalization + hashing
3. Implement `hash_string()` for direct string hashing
4. Ensure consistent hex encoding (lowercase, 64 chars)
5. Write tests
...
mod tests {
    ...
    fn test_verify_hash_case_insensitive() {
        ...
        // verify_hash should work with uppercase too
        // (after normalization in real usage)
        assert!(verify_hash(data, &hash));
    }
...
- [ ] `hash_bytes()` produces 64-character lowercase hex string
- [ ] Same input always produces same hash (deterministic)
- [ ] Different inputs produce different hashes
- [ ] `hash_canonical()` combines canonicalization + hashing
- [ ] Different key order produces same hash after canonicalization
- [ ] Float values cause error in `hash_canonical()`
- [ ] Known hashes match expected values
- [ ] `verify_hash()` correctly validates hashes
- [ ] All tests pass

## Verification

```bash
cargo test --package esrp-canonical hash
...
- Use constant-time comparison for `verify_hash()` to prevent timing attacks
- Always use lowercase hex for consistency
- The `sha2` crate is the de facto standard for Rust SHA256
- Consider adding streaming hash support for large files in future
- The known hash tests ensure cross-platform compatibility
