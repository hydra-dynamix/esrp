# Ticket 206: Write Canonical Tests

**Phase:** 2 - Canonical Representation
**Priority:** High
**Dependencies:** [202 - Implement Canonical JSON](202-implement-canonical-json.md), [203 - Implement SHA256 Hashing](203-implement-hashing.md), [204 - Implement Payload Hash](204-implement-payload-hash.md)
**Blocked By:** Tickets 202, 203, 204

## Summary

Write comprehensive tests for the `esrp-canonical` crate covering edge cases, determinism, and cross-platform compatibility. These tests ensure the canonical implementation is correct and matches the specification.

## Context

Canonical JSON tests are critical because:
- Any bug here breaks idempotency across the entire system
- Cross-platform consistency is essential
- Edge cases in JSON serialization are subtle
- Hash stability must be guaranteed

## Tasks

1. Create test file structure
2. Write key sorting tests
3. Write float rejection tests
4. Write hash determinism tests
5. Write string escaping tests
6. Write edge case tests
7. Create property-based tests (optional)
...
└── tests/
    ...
    ├── hash_test.rs           # Hash tests
    ├── payload_test.rs        # Payload hash tests
    └── edge_cases_test.rs     # Edge case tests
```

### tests/canonical_test.rs
...
mod float_rejection {
    ...
    fn test_large_integer_accepted() {
        let value = json!({"big": 9007199254740991i64});
        let result = to_canonical_json(&value);
        assert!(result.is_ok());
    }
    ...
    fn test_scientific_notation_rejected() {
        // 1e10 is parsed as float by serde_json
        let value: serde_json::Value = serde_json::from_str(r#"{"x": 1e10}"#).unwrap();

        // This may or may not be a float depending on the value
        // 1e10 fits in i64, so it might be parsed as integer
        // Test the canonical behavior regardless
        let result = to_canonical_json(&value);
        // If it's treated as float, should be rejected
        // If treated as int, should be accepted
    }
}
...
use esrp_canonical::{hash_bytes, hash_string, hash_canonical, verify_hash};
use serde_json::json;
...
- [ ] Key sorting tests pass for simple, nested, and Unicode cases
- [ ] Float rejection tests cover all scenarios
- [ ] Array order preservation tests pass
- [ ] No whitespace in output
- [ ] String escaping matches RFC 8259
- [ ] Determinism tests pass (100 iterations)
- [ ] Known hash values match
- [ ] All tests pass with `cargo test`

## Verification

```bash
cargo test --package esrp-canonical

# Run with verbose output
cargo test --package esrp-canonical -- --nocapture
```

## Notes

- Include known hash values for cross-platform verification
- Test Unicode edge cases thoroughly
- Consider adding fuzzing in future (`cargo fuzz`)
